...
class RefereeRuntime:
    ...
    def pause_rotation(self) -> None:
        with self._lock:
            ...
            if state["status"] != "running":
                return
            action_id = self.db.begin_lifecycle_action(
                action_type="pause",
                source_series=int(state["current_series"]),
            )
            now = datetime.now(UTC).isoformat()
            self.db.set_competition_state(status="paused", next_rotation=None, fault_reason=None)
            self.db.set_competition_paused_at(now)
            self._disable_rotation_job()
            self.db.add_event(event_type="admin_action", severity="info", detail="Rotation paused")
            self.db.complete_lifecycle_action(action_id, status="ok", detail="Paused")
    ...
    def resume_rotation(self) -> None:
        with self._lock:
            ...
            try:
                ...
                self._disable_rotation_job()
                self.db.complete_lifecycle_action(action_id, status="failed", detail=str(exc))
                raise
            self.db.set_competition_state(
                status="running",
                fault_reason=None,
            ...
            self.db.set_competition_paused_at(None)
            self._sync_haproxy_active_series(series)
            self._arm_rotation_from_now()
            self.db.add_event(event_type="admin_action", severity="info", detail="Rotation resumed")
            self.db.complete_lifecycle_action(action_id, status="ok", detail=f"Resumed on H{series}")
    ...
    def restart_current_series(self) -> None:
        with self._lock:
            ...
            rotation_started_at = datetime.now(UTC).isoformat()
            self.db.set_competition_state(status="rotating", previous_series=series, fault_reason=None)
            self.db.set_competition_rotation_started_at(rotation_started_at)
            self._run_compose_parallel(
            ...
            try:
                ...
                self.db.set_competition_rotation_started_at(None)
                self._disable_rotation_job()
                self._log_event_and_webhook(
                    event_type="rotation",
                    severity="critical",
            ...
            self.db.complete_lifecycle_action(action_id, status="ok", detail=f"Restarted H{series}")
    ...
    def rotate_to_series(self, target_series: int) -> None:
        with self._lock:
            ...
            action_id = self.db.begin_lifecycle_action(
                action_type="rotate",
                source_series=current_series if current_series > 0 else None,
            ...
            if current_series > 0:
                ...
                self.db.set_competition_rotation_started_at(rotation_started_at)
                # Mark target maint; leave current series in drain (still up) until
                # target is validated.  Current series torn down only after commit.
            ...
            # ---------------------------------------------------------------
            # Safe rotation: deploy target FIRST, validate, THEN tear down
            # source.  This prevents leaving the event with no active series.
            # ---------------------------------------------------------------
            try:
                self._deploy_series_or_raise(series=target_series)
            except RuntimeGuardError as exc:
                # Target failed — clean it up and attempt rollback to source.
                ...
                if current_series > 0:
                    try:
                        ...
                        self.db.complete_lifecycle_action(
                            action_id, status="failed",
                            detail=f"Rotation and rollback both failed: {rollback_exc}",
                        )
                        raise RuntimeGuardError(
                            f"Rotation to H{target_series} failed; rollback to H{current_series} also failed"
                        ) from rollback_exc

                    # Rollback to current_series succeeded — source is still running.
                    ...
                    self.db.complete_lifecycle_action(
                        action_id, status="rolled_back",
                        detail=f"Target failed, rolled back to H{current_series}: {exc}",
                    )
                    return

                # No previous series to roll back to.
                self.db.set_competition_state(
                    status="faulted",
                    current_series=current_series,
                    previous_series=current_series,
            ...
            # Target is healthy — NOW tear down the old series.
            if current_series > 0:
                self._run_compose_parallel(
                    current_series, f"{SETTINGS.docker_compose_cmd} down -v --remove-orphans"
                )
            ...
            self.db.complete_lifecycle_action(
                action_id, status="ok",
                detail=f"Committed H{target_series}",
            )
    ...
    def poll_once(self) -> None:
        with self._lock:
            ...
            # Record per-cycle health facts for debugging.
            healthy_node_count = self._healthy_running_host_count(snapshots)
            degraded_nodes = sorted({
                snap.node_host for snap in snapshots if snap.status == "degraded"
            })
            healthy_variant_names = [
                v for v in SETTINGS.variants
                if healthy_counts.get(v, 0) >= SETTINGS.min_healthy_nodes
            ]
            quorum_met = bool(healthy_variant_names) and not matrix_issues
            self.db.record_node_health_cycle(
                poll_cycle=poll_cycle,
                series=series,
                healthy_nodes=healthy_node_count,
                healthy_variants=healthy_variant_names,
